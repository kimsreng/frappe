		...
		filters.forEach(filter => {
			...
			if (filter.fieldnames) {
				options_html = filter.options.map((option, i) =>
					`<li>
						<a class="dropdown-item" data-fieldname="${filter.fieldnames[i]} data-option="${option}">${__(option)}</a>
					</li>`).join('');
			} else {
				options_html = filter.options.map( option => `<li><a class="dropdown-item" data-option="${option}">${__(option)}</a></li>`).join('');
			}
			...
			$chart_filter.find('.dropdown-menu').on('click', 'li a', (e) => {
				...
				let selected_item = $el.attr("data-option");
				$el.parents(`.${button_class}`).find('.filter-label').text($el.text());
