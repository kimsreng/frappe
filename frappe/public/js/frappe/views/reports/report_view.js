...
frappe.views.ReportView = class ReportView extends frappe.views.ListView {
		...
		if (this.report_name) {
			return this.get_report_doc()
				.then(doc => {
					...
					this.page_title = __(frappe.remove_abbr(this.report_name));
			...
			this.column_widths = this.view_user_settings.column_widths;
		}
	}
	...
	setup_paging_area() {
		...
		if (frappe.is_mobile()) {
			this.$paging_area.find('.level-left').parent().parent().append(
				`<div><span class="comparison-message text-muted">${message}</span></div>`
			);
		} else {
