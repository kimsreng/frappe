...
frappe.views.DashboardView = class DashboardView extends frappe.views.ListView {
	...
	setup_defaults() {
		return super.setup_defaults()
			.then(() => {
				this.page_title = __('{0} Dashboard', [__(this.doctype, null, frappe.trans_context("Doctype", this.doctype))]);
	...
	show_add_chart_dialog() {
		...
		const dialog = new frappe.ui.Dialog({
			title: __("Add a {0} Chart", [__(this.doctype, null, frappe.trans_context("Doctype", this.doctype))]),
			fields: [
				{
