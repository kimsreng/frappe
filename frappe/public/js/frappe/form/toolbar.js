...
frappe.ui.form.Toolbar = class Toolbar {
	...
	set_title() {
		if (this.frm.is_new()) {
			var title = __('New {0}', [__(this.frm.meta.name, null, this.frm.meta.translation_context)]);
		...
		var me = this;
		title = __(frappe.remove_abbr(title));
	...
	make_menu_items() {
		...
		const print_settings = frappe.model.get_doc(":Print Settings", (frappe.boot.agent?  frappe.boot.agent: 'Print Settings')) || {};
